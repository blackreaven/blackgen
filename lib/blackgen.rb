...
require "blackgen/models/thread_generator"
require "commander"
  ...
  class BlackgenApplication
    ...
    def run
      ...
      command :thread do |c|
        ...
        c.option '-M', '--max SIZE', Integer, 'Set max size'
        c.option '-c', '--charset CHARSET', String, 'Set max size'
        c.option '-n', '--thread NUMBER', Integer, 'Set max size'
        ...
        c.action do |args, options|
          ...
          generator = ThreadGenerator.new(options.charset, options.min, options.max, options.thread)
